...
function queryCoreReportingApi(profileId) {
  ...
  .then(function(response) {
	...
	$.ajax({
		url: 'saveGoogleReport.php', // point to server-side PHP script 
		dataType: 'json',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: true,
		data: { formattedJson: formattedJson },
		type: 'get',
		success: function(json){										
			if (json.success) {
				document.getElementById('query-output2').value = json.reportStr;	
			}					
		}
	 });
  })
