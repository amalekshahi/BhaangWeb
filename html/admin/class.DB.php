...
class DB
{
    ...
    public function Execute()
    {
        ...
        if( mysql_errno(self::$_link) )
            throw new Exception(mysql_error(self::$_link), mysql_errno(self::$_link));

        $this->_lastInsertId = (int)@mysql_insert_id(self::$_link);
        $this->_affectedRows = (int)@mysql_affected_rows(self::$_link);
        ...
        for( $i = 0 ; $i < count($this->_vars) ; $i++ )
        {
            ...
            switch( $this->_params[$i] )
            {
                ...
                case 'ignore':
                    ...
                    break;
                default: // string
                    $var = '\''. mysql_real_escape_string($var, self::$_link) .'\'';
            };
